...
void setup()
{
  ...
  button1.begin(button1_pin);                    //Set button 1 pin
  button1.setShortPressCallback(callback1);      //Add a callback when button 1 is short pressed and release
  button1.setLongPressCallback(callback2, 2000); //Add a callback when button 2 is long pressed (doesn't need release)

  button2.begin(button2_pin);
  button2.setShortPressCallback(callback3);
  button2.setLongPressCallback(callback4, 2000);
}
